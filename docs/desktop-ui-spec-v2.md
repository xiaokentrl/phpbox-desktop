...
### 11.9 路线图提前裁决（2026-09-14）

1. **嵌入式终端从 v2 提前实装**（e42c726）：§9 v2 行的引擎前提（server 模式）经分析实际只服务多机管理（远程机需要引擎 daemon）；嵌入式终端仅依赖本地 PTY + bash，与引擎演进无关——同 §11.4 "i18n 从阶段三提前到 v1.1"的提前例。实装形态：linux/darwin 真 PTY（engine/terminal），Windows 诚实降级（stub 明确报错"暂不支持，请用系统终端"）；xterm.js 6 + addon-fit，视图 v-show 常驻（切路由保会话与滚动历史，daemon 前例）；字节双向原样（键入/输出不吞不修饰）。托盘亦属 v2 行且早已随 Wails v3 实装；**多机管理维持 v2 不动**——它是真需要 server 模式的项，GUI 不提前假装。
2. **§6.3 陈旧锁恢复 GUI 侧形态落地**（ac4ecfb）：引擎锁与半安装清理入口仍属 Go 引擎期；GUI 可诚实记录自身任务生命周期（.task-state 文件：发起写入/阶段回写/终结清除），下次启动残留 = 中断证据 → 通知中心报事实（命令/最后阶段/时间）+「请核实或重跑」，不自动重跑不假装能清理。
