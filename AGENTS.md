...
**v0.1 待办**：Windows 构建验证、README。已完成的 v0.1 项：站点健康探活（ec894e0）、命令面板 ⌘K（a50ffae）、绑定层路径统一读 .env（b62f0c7）、Dock/desktop file 安装（用户级 `build/linux/install-user.sh`：`~/.local/bin` + hicolor 图标 + `~/.local/share/applications`；desktop file 含 `StartupWMClass=phpbox-desktop`，生成任务行级追加该键，实测 WM_CLASS=二进制 basename 精确匹配）。
